...
func fillUpTree(
	...
	for _, gongStruct := range sliceOfGongStructsSorted {
		...
		case "Button":
			...
			set := *models.GetGongstructInstancesSet[models.Button](playground.stageOfInterest)
			for button := range set {
				nodeInstance := (&gongtree_models.Node{Name: button.GetName()}).Stage(playground.treeStage)
				nodeInstance.IsNodeClickable = true
				nodeInstance.Impl = NewInstanceNodeCallback(button, "Button", playground)
		...
		case "Node":
			...
			for node := range set {
				nodeInstance := (&gongtree_models.Node{Name: node.GetName()}).Stage(playground.treeStage)
				nodeInstance.IsNodeClickable = true
				nodeInstance.Impl = NewInstanceNodeCallback(node, "Node", playground)

				nodeGongstruct.Children = append(nodeGongstruct.Children, nodeInstance)
			}
		case "Tree":
			nodeGongstruct.Name = name
			set := *models.GetGongstructInstancesSet[models.Tree](playground.stageOfInterest)
			for tree := range set {
				nodeInstance := (&gongtree_models.Node{Name: tree.GetName()}).Stage(playground.treeStage)
				nodeInstance.IsNodeClickable = true
				nodeInstance.Impl = NewInstanceNodeCallback(tree, "Tree", playground)
